...
8. **🚨 数据迁移铁律: 禁止 rsync --remove-source-files 到远程** — 写入失败会静默丢失数据(2026-08-31 丢失 7.2G 教训), 必须分两步: 先复制验证再手动删除
9. **🚨 迁移后必须校验哈希才能删源文件** — 复制完成后, 必须逐文件比对源和目标的 SHA256 哈希值, 全部一致后才能删除源文件; 哈希不一致 = 迁移未完成, 绝对不能删
10. **🚨 任何删除前必须留证** — 不只是磁盘清理, 任何时候删除文件/目录前, 必须先生成删除清单(目录树+文件列表+SHA256哈希+描述)存档到安全位置, 以便追溯和恢复
...
- **WebDAV 写入不可靠** — 迁移数据优先用 SMB/NFS，WebDAV 只适合读
